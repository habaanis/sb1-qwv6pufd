...
export default function CategorySearchBar({
  ...
  const fetchSuggestions = async () => {
    try {
      let dbQuery = supabase
        ...
        .select('id, nom, ville, gouvernorat, sous_categories, "mots cles recherche", "liste pages"')
        .limit(8);

      if (listePageValue) {
        dbQuery = dbQuery.contains('"liste pages"', [listePageValue]);
      }

      dbQuery = dbQuery.or(`nom.ilike.%${query}%,sous_categories.ilike.%${query}%,"mots cles recherche".ilike.%${query}%`);

      const { data, error } = await dbQuery;
      console.log('DEBUG SUGGESTIONS:', { data, error, query, listePageValue });
      if (!error && data) {
  ...
  const fetchVilleSuggestions = async () => {
    try {
      let dbQuery = supabase
        .from(Tables.ENTREPRISE)
        .select('gouvernorat, "liste pages"')
        .ilike('gouvernorat', `%${ville}%`)
        .limit(10);
      ...
      const { data, error } = await dbQuery;
      if (!error && data) {
        const uniqueVilles = [...new Set(data.map((d: any) => d.gouvernorat).filter(Boolean))];
        ...
        <div className="lg:w-64 relative" ref={villeRef}>
          <div className="relative">
            ...
            <input
              ...
              onKeyPress={handleKeyPress}
              onFocus={() => ville.length >= 2 && setShowVilleSuggestions(true)}
