/* ---------------------------------------------------------
   ...
   Projet : Dalil Tounes 🇹🇳
   Objectif : Connexion Supabase + fonction compteur dynamique
--------------------------------------------------------- */
...
const supabaseUrl = "https://kmvjegbtroksjqaqliyv.supabase.co";
const supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImttdmplZ2J0cm9rc2pxYXFsaXl2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE4MDA1NTEsImV4cCI6MjA2NzM3NjU1MX0.MbU7b-HWQBwlYtbJeE7_ABvrGhuhzeAuqvkcVvvoE1o";
...
(async () => {
  const { data, error } = await supabase.from('entreprise').select('nom').limit(1);
  if (error) {
    console.error("❌ Erreur Supabase (test connexion) :", error.message);
  } else {
    console.log("✅ Supabase Dalil Tounes connecté :", data?.[0]?.nom || "Connexion OK");
  }
})();
...
--------------------------------------------------------- */
export async function searchEducation({ keyword = '', city = '', quick = null }) {
  try {
    let query = supabase.from('entreprise').select('*');

    // Filtrer par catégorie éducation par défaut
...
export async function addTeacher(payload) {
  try {
    ...
    if (error) {
      console.error('❌ Erreur addTeacher:', error.message);
      return { error };
    }
    return { data, error: null };
  } catch (err) {
    console.error('❌ Erreur critique addTeacher:', err.message);
    return { error: err };
  }
}
...
export async function getFeaturedEvents(limit = 10) {
  try {
    ...
    if (error) {
      console.error('❌ Erreur getFeaturedEvents:', error.message);
      return [];
    }
    ...
    console.error('❌ Erreur critique getFeaturedEvents:', err.message);
    return [];
  }
}
...
export async function getAllEvents() {
  try {
    ...
    console.error('❌ Erreur critique getAllEvents:', err.message);
    return [];
  }
}
...
export async function searchHealthProfessionals(opts) {
  const { specialty = '', city = '', acceptCNAM = false, limit = 30 } = opts;

  try {
    let q = supabase
      .from('entreprise')
      .select('id, nom, ville, categories, sous_categories, telephone, site_web, photo_url, accepte_cnam', { count: 'exact' })
      .eq('status', 'approved')
      .limit(limit);

    // Nuage santé côté catégories (liste étendue)
    const healthCloud = [
      'santé', 'santé & urgence', 'clinique', 'hopital', 'hôpital',
      'médecin', 'medecin', 'cardiologie', 'dentiste', 'dentaire',
      'radiologie', 'pédiatre', 'pediatre', 'pharmacie', 'laboratoire',
      'kiné', 'kinesitherapeute', 'dermatologie', 'gynécologie',
      'ophtalmologie', 'orl', 'urologie', 'orthopédie',
      'psychiatrie', 'psychologie'
    ];

    // Restreindre aux entrées du domaine santé
    const healthFilters = healthCloud.map(h => `categories.ilike.%${h}%`).join(',')
      + ',' + healthCloud.map(h => `sous_categories.ilike.%${h}%`).join(',');

    q = q.or(healthFilters);

    // Filtre par spécialité
    if (specialty.trim()) {
      const s = specialty.trim();
      q = q.or(`nom.ilike.%${s}%,categories.ilike.%${s}%,sous_categories.ilike.%${s}%,description.ilike.%${s}%`);
    }

    // Filtre par ville
    if (city.trim()) {
      q = q.ilike('ville', `%${city.trim()}%`);
    }

    // Filtre CNAM
    if (acceptCNAM) {
      q = q.eq('accepte_cnam', true);
    }
...
/* ---------------------------------------------------------
   🏙️ Fonction : Récupérer les villes depuis Supabase (autocomplétion)
--------------------------------------------------------- */
export async function searchCities(searchTerm) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('name_fr, name_ar, governorate_fr')
